...
public class Wizard extends Character implements Action {
    //기본 스펙 생각하기
    // 물약 먹기 구현
    // 스킬명도 생각해두고, 어떻게 할건지
    ...
    public Wizard(String gender, String characterName, double attackPower, double defense, double stamina, double MP) {
    ...
    public void run() { //50퍼센트 확률로 도망 성공 (무승부) or 공격력 감소
        ...
        if(Math.random()<0.5){ //랜덤으로 0~1까지의 모든 실수를 받아옴. 50퍼센트 확률
            ...
            System.out.println("현재 공격력 : "+getAttackPower());
        }
    }
    ...
    public void skill(Character c) {
        ...
        System.out.println(getCharacterName()+" : 파이어볼!");
        System.out.println(getCharacterName()+"가 파이어볼을 사용했습니다.");
        //현재 체력에서 감소된 체력 보여주기
        //현재 체력 : c.getStamina
        //감소된 체력 : 현재체력 - 1.8*(공격력 - 방어력/2)
        System.out.printf("체력 : %.1f -> %.1f \n",c.getStamina(),c.getStamina()-(c.getAttackPower()-c.getDefense()/2)*1.8);
    ...
    public void mockery(Character c) {
        System.out.println(getCharacterName()+" : 에베베벱~~@$#=^오^=");
